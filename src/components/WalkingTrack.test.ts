...
describe('WalkingTrack.vue', () => {
  ...
  it('should render when game starts', async () => {
    ...
    expect(wrapper.attributes('style')).toContain('min-height: 1080vh')
    
    // Check if progress bar exists
    expect(wrapper.find('.bg-emerald-200').exists()).toBe(true)
    
    // Check if all available PostcardItem components are rendered
    const postcards = wrapper.findAllComponents(PostcardItem)
    expect(postcards).toHaveLength(14)
    ...
    expect(postcards[13].props('index')).toBe(13)
